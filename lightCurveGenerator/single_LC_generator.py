...
class Megastructure:
    ...
    def __init__(self, Rorb=1.0, iscircle=False, Rcircle=1.0, isrot=False, ph_offset=0.0,
        ...
        if iscircle:
            ...
            self.Plcoords = np.transpose(
                np.asarray([Rcircle * np.cos(th), Rcircle * np.sin(th), np.zeros(self.circ_res)]))
        else:
            self.Plcoords = Plcoords  # [(x,y,z),(x,y,z)...]
    ...
    def rotate(self, axis, L):
        ...
        n2 = axis[1]
        n3 = axis[2]
        mat = np.array([[np.cos(L) + n1 ** 2 * (1 - np.cos(L)), n1 * n2 * (1 - np.cos(L)) - n3 * np.sin(L),
                         n1 * n3 * (1 - np.cos(L)) + n2 * np.sin(L)],
                        [n1 * n2 * (1 - np.cos(L)) + n3 * np.sin(L), np.cos(L) + n2 ** 2 * (1 - np.cos(L)),
                         n2 * n3 * (1 - np.cos(L)) - n1 * np.sin(L)],
                        [n1 * n3 * (1 - np.cos(L)) - n2 * np.sin(L), n2 * n3 * (1 - np.cos(L)) + n1 * np.sin(L),
                         np.cos(L) + n3 ** 2 * (1 - np.cos(L))]])
    ...
    def translate(self, frm):
        ...
        """
        kep_corr = self.Rorbit * (1 - self.ecc ** 2) / (
                    1 + self.ecc * np.cos(self.o_vel * frm + self.ph_offset - self.periapsis_offset))
...
class Simulator:
    ...
    def Prob(self, x, z, u, v):
        ...
        y = ((1 - u - 2 * v) * x + v * x ** 3 / (3 * a ** 2) + ((u + 2 * v) / a) * (
                    x * np.sqrt(a ** 2 - x ** 2) / 2 + a ** 2 * np.arcsin(x / a) / 2)) / k - z
        return (y)
    ...
    def monte_carlo_multi(self, frame):
        ...
        for meg in self.tmegs:
            if (meg.glp(frame) > np.pi / 2 and meg.glp(frame) < 3 * np.pi / 2):
                dists.append(np.zeros(self.no_pt))
                ...
                if (meg.iscircle and not meg.isrot):
                    ...
                    distarr = np.asarray(
                        in_or_out(self.ran_rad * np.sin(self.ran_th), self.ran_rad * np.cos(self.ran_th),
        ...
        for frame in self.frames:
            for i in range(len(self.megs)):
                if self.megs[i].isrot:
                    tcoordsh = self.megs[i].rotate(self.megs[i].rot_axis,
                                                   self.megs[i].rot_vel * frame + self.megs[i].ph_offset)

                else:
                    tcoordsh = self.megs[i].Plcoords
...
class Transit_Animate:
    ...
    def init_frame(self):
        ...
        self.ax2.set_xticklabels(
            ['-2π', '-7π/4', '-3π/2', '-5π/4', '-π', '-3π/4', '-π/2', '-π/4', '0', 'π/4', 'π/2', '3π/4', 'π', '5π/4',
             '3π/2', '7π/4', '2π'])
    ...
    def update(self, frame):
        ...
        self.ax1.fill(self.gopath.Rstar * np.cos(self.th), self.gopath.Rstar * np.sin(self.th), zorder=zst,
                      color='#ffae42')
    ...
    def go(self, ifsave=True, filepath="animation.gif"):
        ani = animation.FuncAnimation(self.fig, self.update, frames=np.arange(0, len(self.phase)), interval=1,
...
Rstar_siml = 100
Rmega = 0.5* Rstar_siml  # 0.5 times radius of starS

sim1 = Simulator(Rstar = Rstar_siml, no_pt = 10000,frame_no = 500, frame_length = np.pi/3)
...
meg_2d = Megastructure(Rorb=200, iscircle = True, Rcircle = Rmega, isrot=True, Plcoords=coord_bezier,incl=0, ph_offset=0, elevation=0, ecc=0, per_off=0)
